...
### 数据包处理时延实验
lantency-data目录存放数据包处理时延数据。此部分有三个子目录，分别用于测量①第一种绕过方法在接收方向对数据包处理引入的时延，
...
测量方法是构造UDP数据包，其中UDP的负载是数据包的编号。测量方法会维护一个键值对数据结构。

对于接收方向的数据包，当数据包被`__netif_receive_skb_core`函数处理时，通过内核提供的`ktime_get_ns()`函数获得当前时间，以数据包负载中的编号为键，以当前时间为值，写入键值对数据结构中。
